...
struct TrackInfo
{
    ...
    TrackInfo(const Name &inName, const Title &inTitle, const Path &inTemplatePath, const std::string& cExt, const std::string& pExt)
    {
        pagename = inName;
        title = inTitle;
...
struct SiteInfo
{
    ...
    int track_dir(const Path& dirPath, const std::string& cExt, const Path& templatePath, const std::string& pExt);
    int track(const std::vector<TrackInfo>& pagesToTrack);
    int track(const Name& name, const Title& title, const Path& templatePath, const std::string& cExt, const std::string& pExt);
    int untrack(const Name& pageNameToUntrack);
    int untrack(const std::vector<Name>& pageNamesToUntrack);
    ...
    int rm(const std::vector<Name>& pageNamesToRemove);
